...
## 🚀 提前要求與安裝環境 (Prerequisites)
在使用本專案之前，請確保您的系統符合以下要求：
1. **作業系統**：Windows 10 或 Windows 11（因為使用了 Windows 原生的 Media Control API 獲取音樂資訊）。
2. **Python 環境**：安裝 Python 3.9 或以上版本。
3. **Node.js 環境**：如需使用網頁管理後台，請安裝 Node.js (建議 v18+)。
4. **音樂播放軟體**：支援 Windows 媒體控制的播放器（如 Spotify、Apple Music、網易雲音樂、Youtube Music 桌面版等）。
...
### 4. 網頁版管理後台 (Web-App)
提供進階的歌詞編輯與個人聽歌數據統計。
*   **啟動後台**：在 \web-app\ 目錄下執行 \
pm install\ 與 \
pm start\，然後在瀏覽器輸入 \http://localhost:3000\。
...
## 🧩 核心依賴套件說明 (Dependencies)
本專案的架構設計為**模組化**，您可以根據自身需求選擇性安裝環境，以下是各主要依賴的用途說明：

### 1. Python 核心依賴 (\equirements.txt\)
*   **\PyQt6\**：負責繪製透明、無邊框的桌面懸浮歌詞 UI 介面。
*   **\winrt\**：攔截 Windows 系統底層的「媒體控制項」，藉此實現**免金鑰、免授權**即可支援 Spotify、Apple Music 等所有音樂播放器。
*   **\syncedlyrics\**：用於從各大平台（如網易雲、Lrclib 等）自動搜尋並抓取動態時間軸歌詞。
*   **\pykakasi\**：處理日文分詞與平假名轉換，是實現「自動假名注音 (Furigana)」的核心。
*   **\keyboard\**：提供全域快捷鍵功能，讓您即使在背景也能操控歌詞視窗。

### 2. 可選模組環境
如果您覺得環境太過龐大，只需執行 Python 核心的 \main.py\ 即可擁有最完整的桌面歌詞體驗，以下模組皆為**可選**：
*   **Node.js (網頁管理後台)**：用於提供瀏覽器介面與歷史數據統計。若不啟動，完全不影響桌面版運作。
*   **.NET 8.0 (靈動島介面)**：靈動島 (\DynamicIslandUI\) 是由 C# WPF 開發的原生 Windows 應用程式，依賴微軟的 .NET 8.0 執行環境來確保 60FPS 的流暢動畫。若您不需要靈動島視覺效果，**完全可以不裝 .NET 8.0**。
